...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchProducts() {
        try {
            const response = await fetch('http://13.233.153.178:5004/api/products'); // Ensure this URL is correct
            if (!response.ok) {
...
salesForm.addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch('http://13.233.153.178:5004/api/sales', {  // Ensure the URL is correct
            method: 'POST',
            headers: {
    ...
    generateReportBtn.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch(`http://13.233.153.178:5004/api/sales/report?startDate=${startDate}&endDate=${endDate}`);
